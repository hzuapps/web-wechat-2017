...
Page({
  ...
  onLoad: function (options) {
    ...
    app.getRes(`book/${options.id}`).then((data) => {
      data.cover = data.cover ? app.getImageUrl(data.cover) : '../../images/default_book_cover.png'
    ...
    app.getRes(`book/${options.id}/recommend`).then((data) => {
      ...
      data.books.slice(0, 4).map(item => {
        item.cover = item.cover ? app.getImageUrl(item.cover) : '../../images/default_book_cover.png'
